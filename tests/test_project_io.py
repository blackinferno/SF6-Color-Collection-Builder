...
from app.exporter import export_collection_archive, export_collection_zip
...
def test_load_exported_collection_7z(tmp_path: Path) -> None:
    source_zip = tmp_path / "Source.zip"
    source_internal = "x/esf001_001_cmd_002.user.2"
    import zipfile
    ...
    output_archive = tmp_path / "Loaded Collection.7z"
    export_collection_archive(output_archive, "Loaded Collection", [assignment])
    ...
    assert mods_folder == str(tmp_path)
    assert assignments[0].target_slot == "004"
    assert assignments[0].source_kind == "7z"
