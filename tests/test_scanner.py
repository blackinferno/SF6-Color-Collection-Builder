...
def test_scan_zip_detects_modinfo_preview_and_color_files(tmp_path: Path) -> None:
    ...
    _write_zip(
        zip_path,
        {
            ...
            "CoolMod/nested/esf001_001_cmd_002.user.2": b"normal",
            "CoolMod/nested/esf001_001_cmd_dx_003.user.2": b"dx",
            "CoolMod/nested/esf001_001_cmd_ex_011.user.2": b"invalid slot",
        },
    )
...
def test_scan_mods_folder_only_scans_top_level_zips(tmp_path: Path) -> None:
    _write_zip(
        tmp_path / "Valid.zip",
        {"modinfo.ini": "name=Valid\n", "x/y/esf002_003_cmd_010.user.2": b"ok"},
    )
    ...
    _write_zip(
        nested / "Nested.zip",
        {"modinfo.ini": "name=Nested\n", "esf001_001_cmd_001.user.2": b"ignored"},
    )
...
def test_scan_mods_folder_splits_submods_with_overlapping_slots(tmp_path: Path) -> None:
    _write_zip(
        tmp_path / "DXEX Colors.zip",
        {
            ...
            "DX/esf001_001_cmd_dx_001.user.2": b"dx",
            "EX/modinfo.ini": "name=EX Colors\nauthor=Creator\n",
            "EX/esf001_001_cmd_dx_001.user.2": b"ex package dx slot",
        },
    )
    ...
    assert mods[0].source_for("esf001", "001", "dx", "001").internal_file_path == (
        "DX/esf001_001_cmd_dx_001.user.2"
    )
    assert mods[1].source_for("esf001", "001", "dx", "001").internal_file_path == (
        "EX/esf001_001_cmd_dx_001.user.2"
    )
